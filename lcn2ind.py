...
# Extract data
eeg_data = mat_data['LCN']['trials'][0][0][185:397,:,:]  # Adjust the indices based on your file contents
...
baseline_mean=np.mean(baseline_period,axis=2,keepdims=True)
...
for i in range(powered_avg.data.shape[0]):
    ...
    plt.title(f'LCN Power Spectrogram - Rat_id: 3 '+ch_names[i])  # Set the title for the plot
